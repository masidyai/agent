"""
CodeExecution model for tracking Docker-based code execution sessions
"""
import uuid
...
class CodeExecutionPhase(str, Enum):
    """Code execution pipeline phases"""
...
class CodeExecution(Base):
    """CodeExecution model for tracking Docker code execution sessions"""
    ...
    status: Mapped[CodeExecutionStatus] = mapped_column(
        SQLEnum(CodeExecutionStatus),
        default=CodeExecutionStatus.PENDING,
    )
    current_phase: Mapped[Optional[CodeExecutionPhase]] = mapped_column(
        SQLEnum(CodeExecutionPhase),
        nullable=True,
    )
    ...
    # Relationships
    project: Mapped["Project"] = relationship("Project", back_populates="code_executions")
    
    def __repr__(self) -> str:
        return f"<CodeExecution {self.id} - {self.status}>"
